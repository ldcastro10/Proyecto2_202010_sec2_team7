...
public class MVCView 
{
	...
	public void printReq2C(ArregloDinamico arr) 
	{
		...
		try 
		{
			Date date = spf.parse("2017/12/31");
			
			int i = 0;
			
			while (date.compareTo(spf.parse("2019/01/01")) < 0 && datos.darTamano() != i)
			{
				Calendar c = Calendar.getInstance();
				c.setTime(date);				
				...
				for (int j = 0; j < datosPorFecha.darElemento(0); j++) 				
					System.out.print("*");				

				System.out.println();
				System.out.println("           |");
				
				for (int j = 0; j < datosPorFecha.darElemento(1); j++) 				
					System.out.print("#");				
